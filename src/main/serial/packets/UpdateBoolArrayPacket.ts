...
export const UpdateBoolArrayPacketType: SerialPacketType<UpdateValuePacket<boolean[]>> = {
    typeID: SerialPacketTypeID.UPDATE_BOOL_ARRAY,
    serialize: (packet) => {
        const payload = Buffer.alloc(4 + packet.value.length);
        payload.writeUInt16BE(packet.ntID, 0);
        payload.writeUInt16BE(packet.value.length, 2);
        for (let i = 0; i < packet.value.length; i++)
            payload.writeInt8(packet.value[i] ? 1 : 0, 4 + i);
    ...
    deserialize: (packet) => {
        ...
        for (let i = 0; i < length; i++)
            value.push(packet.payload.readInt8(4 + i) !== 0);
